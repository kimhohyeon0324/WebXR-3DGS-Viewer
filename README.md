...
### 2) 3DGS 뷰어에 최적화된 WebXR 6DoF 오브젝트 인스펙션 파이프라인 구축
* **문제 현상**:
  - 3DGS 라이브러리는 본래 마우스 궤도 회전 뷰어로만 설계되어 있어, WebXR 세션 진입 시 3D 모델을 손으로 직접 쥐고 돌려보거나 크기를 조절하는 6DoF 컨트롤러 인터랙션이 전혀 동작하지 않는 문제 발생.
  - 가상 보행이나 텔레포트는 소형 3D 스캔 객체를 검사하는 목적에 불필요하며, 복잡한 양손 조합이나 빈 공간 클릭 시 시점 리셋 오작동이 사용자 몰입을 저해.
* **해결 방법**:
  - 뷰어 목적에 특화된 직관적인 **인스펙터(Inspector) 조작계** 구축.
  - **직관적 그랩 & 손목 회전**: 어느 손이든 그립을 쥔 채 움직이면 모델이 1:1로 손을 따라 이동하고 손목 회전(Yaw)에 맞춰 회전.
  - **썸스틱 부드러운 회전 & 줌**: 썸스틱 좌/우 조작으로 모델을 부드럽게 회전시키고, 상/하 조작으로 마우스 휠처럼 즉각 줌인/줌아웃.
  - **양손 핀치 줌**: 양손 그립을 쥔 상태에서 두 손을 벌리거나 좁혀 직관적인 스케일 조절 지원.
  - **트리거 단독 POI 핀 피킹**: 불필요한 시점 복귀 오작동을 완전히 제거하고, 트리거는 3D POI 어노테이션 핀의 레이저 조준 및 피킹에만 집중하도록 격리.
...
| 컨트롤러 입력 | 동작 |
| :--- | :--- |
| **양손 그립 (누른 채 이동/회전)** | 3DGS 모델 잡고 이동 및 손목 각도에 따른 회전 |
| **오른손/왼손 썸스틱 (좌/우)** | 3DGS 모델 부드러운 연속 회전 |
| **오른손/왼손 썸스틱 (상/하)** | 3DGS 모델 실시간 줌인 / 줌아웃 |
| **양손 그립 (동시 누름)** | 양손 핀치 줌 (두 손 간격으로 모델 확대/축소) |
| **트리거 (클릭)** | 3D POI 핀 레이저 조준 및 어노테이션 카드 피킹 |
