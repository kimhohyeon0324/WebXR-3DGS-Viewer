...
| 조작 | 기능 | 설명 |
| :--- | :--- | :--- |
| **왼쪽 Trigger 누른 채 이동** | **상하 회전 (Pitch)** | 모델을 위아래로 기울입니다. |
| **오른쪽 Trigger 누른 채 이동** | **좌우 회전 (Yaw)** | 모델을 좌우로 돌립니다. |
| **양손 Trigger 동시 사용** | **자유 회전** | 양손을 이용해 상하좌우를 동시에 회전합니다. |
| **Grip 누른 채 이동** | **위치 이동 (Pan)** | 모델의 위치를 상하좌우로 이동합니다. |
| **양손 Grip 동시 조작** | **확대/축소 (Zoom)** | 두 손의 간격을 벌리거나 좁혀 모델 크기를 조절합니다. |
| **Thumbstick 앞/뒤** | **확대/축소 (Zoom)** | 썸스틱으로 간편하게 모델을 확대/축소합니다. |
| **POI 핀 조준 + Trigger 클릭** | **정보 카드 열람** | 3D 핀을 클릭해 부가 정보를 확인합니다. |
| **B 또는 Y 버튼** | **모델 변경** | 프리셋 모델(`Bonsai`, `Dragon`)로 즉시 전환합니다. |
| **A 또는 X 버튼** | **시점 리셋** | 모델을 내 정면 눈높이로 다시 불러옵니다. |
