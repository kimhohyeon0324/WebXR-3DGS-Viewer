...
* **해결 방법**:
  - 뷰어 목적에 부적합한 가상 보행 대신, 공중에 뜬 3D 모델을 손으로 직접 검사하는 **인스펙터(Inspector) 조작계** 구축.
  - **원핸드 그랩 & 손목 회전**: 오른손 그립을 쥔 채 움직이면 컨트롤러의 이동량 및 손목 각도 변화에 따라 3DGS 모델이 즉각 이동·회전하도록 바인딩.
  - **양손 핀치 줌**: 양손 그립을 쥐고 두 손을 벌리거나 좁힐 때 실시간 상대 거리를 산출하여 모델을 0.2~5.0배 범위로 자유롭게 확대·축소.
  - **원터치 정면 시점 복귀 (Recenter)**: 모델이 시야를 벗어났을 때 트리거 클릭 또는 A 버튼으로 현재 사용자 눈높이 정면(1.2m 앞)으로 즉시 재정렬.
  - **45° 스냅 회전**: 오른손 썸스틱 좌우 조작으로 모델을 45도 단위로 정밀하게 회전 검사.
...
| 컨트롤러 입력 | 동작 |
| :--- | :--- |
| **오른손 그립 (누른 채 이동/손목 회전)** | 3DGS 모델 잡고 이동 및 손목 각도로 회전 (Grab Orbit & Pan) |
| **양손 그립 (동시 누름)** | 양손을 벌리거나 좁혀 모델 확대/축소 및 양손 축 회전 (Pinch Scale) |
| **오른손 트리거 (클릭)** | 빈 공간 클릭 시 모델을 현재 시선 정면 눈높이로 즉시 복귀 (Recenter) / POI 핀 피킹 |
| **오른손 썸스틱 (좌/우)** | 모델 45° 스냅 회전 |
| **A 버튼** | 원터치 모델 위치 및 크기 리셋 |
