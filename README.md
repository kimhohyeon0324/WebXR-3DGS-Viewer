...
### Meta Quest (WebXR) 환경 — 데스크톱 1:1 직관적 매핑
| 데스크톱 웹 조작 | VR 컨트롤러 입력 | 물리 동작 및 피드백 | 상세 설명 |
| :--- | :--- | :--- | :--- |
| **마우스 좌클릭 드래그** | **Trigger(검지 방아쇠) 누르고 드래그** | **씬 360° 궤도 회전 (Orbit)**<br>*(보라색 레이저 & 햅틱 펄스)* | 데스크톱 마우스 좌클릭 드래그와 완벽히 동일하게, 검지 트리거를 쥐고 손을 움직이거나 손목을 틀면 오브젝트 중심 기준 360° 매끄럽게 궤도 회전합니다. |
| **마우스 우클릭 드래그** | **Grip(중지 손잡이) 쥐고 드래그** | **카메라 / 모델 위치 이동 (Pan)**<br>*(에메랄드 레이저 & 햅틱 펄스)* | 데스크톱 마우스 우클릭 드래그와 동일하게, 중지 그립 버튼을 쥔 채 컨트롤러를 움직이면 모델의 3차원 위치를 자유롭게 이동시킵니다. |
| **마우스 휠 스크롤** | **Thumbstick(썸스틱) 앞/뒤 조작** | **카메라 확대 / 축소 (Zoom)** | 썸스틱을 앞으로 밀면 모델 확대 (Zoom In), 뒤로 당기면 축소 (Zoom Out)되어 마우스 휠 스크롤과 완벽히 동일한 줌 조작감을 제공합니다. |
| **3D POI 핀 클릭** | **POI 핀 조준 후 Trigger 클릭** | **VR 3D 어노테이션 정보 카드 열람/닫기**<br>*(골드 레이저 & 클릭 햅틱)* | 레이저가 POI 핀을 가리키면 골드 색상으로 빛나며 손끝에 미세 햅틱이 전달되고, 방아쇠를 당기면 3D 정보 카드가 열리고 닫힙니다. |
| **상단 드롭다운 모델 전환** | **B 버튼(우) 또는 Y 버튼(좌) 클릭** | **프리셋 모델 즉시 전환 (`Bonsai` ↔ `Dragon`)**<br>*(전환 햅틱 펄스)* | VR 세션을 종료하지 않고도 HMD 안에서 B/Y 버튼으로 프리셋 모델을 즉시 전환할 수 있습니다. |
| **시점 리셋** | **A 버튼(우) 또는 X 버튼(좌) 클릭** | **시선 정면 즉시 복귀 (Recenter)**<br>*(더블 햅틱 펄스)* | 모델을 놓치거나 시야 밖으로 멀어졌을 때, 현재 사용자의 정면 1.3m 눈높이로 모델을 부드럽게 재배치하고 크기/각도를 초기화합니다. |
| *(보조 제스처)* | **Grip 또는 Trigger 양손 동시 쥐기** | **양손 3D 비틀기 회전 + 3D 핀치 줌**<br>*(양손 동시 햅틱 피드백)* | 양손을 동시에 사용할 경우 두 손의 거리로 핀치 줌 및 두 손의 벡터 축으로 비틀기 회전도 자연스럽게 지원합니다. |

> **인체공학적 설계 원칙 (Ergonomic Principles)**:
> 1. **데스크톱 웹 환경 1:1 일치 (Unified Mental Model)**: 좌클릭(트리거=회전), 우클릭(그립=이동), 휠 스크롤(썸스틱=줌), 클릭(POI 핀)의 조작계가 데스크톱과 일치하여 학습 비용이 0입니다.
> 2. **오브젝트 중심 고정 피벗**: 회전 및 확대/축소의 피벗 축이 항상 모델 자체의 로컬 중심에 고정되어 있어 원거리에서도 모델이 튀거나 시야 밖으로 날아가지 않습니다.
> 3. **물리 지수 감쇠 스무딩 (Exponential Damping)**: 인체의 미세 손떨림을 필터링하여, 마치 고급 물체를 다루듯 매끄럽고 안정적인 조작감을 제공합니다.
