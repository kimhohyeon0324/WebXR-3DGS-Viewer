...
| 컨트롤러 입력 | 동작 |
| :--- | :--- |
| **트리거 / 그립 (누른 채 이동/회전)** | 3DGS 모델 잡고 이동 및 손목 각도에 따른 회전 |
| **트리거 / 그립 (양손 동시 누름)** | 양손 핀치 줌 & 회전 (두 손 간격으로 모델 확대/축소) |
| **오른손/왼손 썸스틱 (좌/우)** | 3DGS 모델 부드러운 연속 회전 |
| **오른손/왼손 썸스틱 (상/하)** | 3DGS 모델 실시간 줌인 / 줌아웃 (마우스 휠 방식) |
| **트리거 (레이저로 POI 핀 조준 클릭)** | **VR 세션 내 3D 어노테이션 정보 카드 열람 / 닫기 토글** |
