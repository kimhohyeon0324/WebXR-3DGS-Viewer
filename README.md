...
| 컨트롤러 입력 | 동작 |
| :--- | :--- |
| **한 손 트리거 (누른 채 컨트롤러 이동)** | 3DGS 모델을 잡고 원하는 위치로 1:1 **자유로운 위치 이동 (Translation)** (손목 회전 간섭 배제) |
| **양손 트리거 (동시 누른 채 조작)** | • **3D 회전**: 양손을 잡고 돌려 **오브젝트 로컬 중심 회전**<br>• **확대/축소**: 두 손 간격을 벌리거나 좁혀 **오브젝트 중심 줌인/줌아웃**<br>• **위치 이동**: 두 손을 함께 움직여 **공간 이동** |
| **트리거 단발 클릭 (레이저로 POI 핀 조준)** | **VR 세션 내 3D 어노테이션 정보 카드 열람 / 닫기 토글** |

> **안내**: 썸스틱 및 그립 버튼의 복잡한 조작을 완전히 배제하고, 마우스처럼 직관적인 **트리거 단독 조작계**로 통일했습니다. 특히 모델의 회전 및 확대/축소 피벗 축은 사용자 시야나 월드 원점이 아닌 **오브젝트 자체의 중심**에 고정되어, 조작 중 모델이 시야 밖으로 날아가거나 튀지 않고 언제나 자연스럽게 조작됩니다.
