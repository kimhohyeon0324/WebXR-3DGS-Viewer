...
1. **정적 모니터 뷰어에 갇혀 있던 3DGS 공간 경험**:
   - 3D 가우시안 스플래팅(3DGS)은 실사 공간을 높은 품질로 복원하는 혁신적인 포맷임에도 불구하고, 기존 웹 뷰어들은 대부분 PC 모니터에서 마우스로 회전해 보는 궤도(Orbit) 뷰어에 머물러 있었습니다.
   - 공간 데이터를 본질적으로 체험하기 위해서는 VR 헤드셋 안에서 사용자가 두 발로 직접 걸어 다니고, 텔레포트하며, 양손으로 공간 크기를 조절할 수 있는 **몰입형 6DoF 인터랙티브 시스템**이 필수적이었습니다.
2. **독립형 모바일 VR(Meta Quest)의 심각한 연산 부하**:
   - 고성능 PC와 달리, 모바일 프로세서(Snapdragon XR2) 기반의 독립형 HMD에서는 수십만 개의 반투명 입자가 중첩될 때 발생하는 연산량(오버드로우)을 감당하지 못해 프레임이 45 FPS 이하로 급락하고 심한 VR 멀미를 유발했습니다.
   - 따라서 **"별도 앱 설치 없이 웹 브라우저로 즉시 접속하면서도, 모바일 VR 기기에서 60~90 FPS를 방어하는 6DoF 인터랙티브 뷰어"**를 개발하고자 본 프로젝트를 시작했습니다.
...
### 2) WebXR 기반 6DoF 공간 조작(텔레포트 및 양손 인터랙션) 파이프라인 구축
* **문제 현상**:
  - 3DGS 라이브러리는 본래 마우스 궤도(Orbit) 뷰어로만 설계되어 있어, WebXR 세션 진입 시 가상 공간 내 보행 이동, 컨트롤러 방향 정렬, 지면 텔레포트 및 양손 공간 변환이 전혀 동작하지 않는 문제 발생.
* **해결 방법**:
  - Three.js 씬 내에 전용 `CameraRig` 계층 구조를 설계하여, HMD 헤드 트래킹 좌표계와 가상 씬 좌표계를 분리 바인딩.
  - 오른손 썸스틱 입력에 반응하는 **포물선 궤적 텔레포트(Parabolic Teleport)** 알고리즘과 45° 스냅턴을 구현하여 공간 이동 시 VR 멀미를 최소화.
  - 양손 그립 버튼 입력 시 두 컨트롤러 간의 실시간 상대 거리와 회전 벡터를 산출하여, 3DGS 공간 전체를 직관적으로 확대·축소하고 회전할 수 있는 **양손 조작(Bimanual Transformation) 시스템**을 직접 구현.
