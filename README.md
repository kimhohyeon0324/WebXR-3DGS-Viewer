...
### Meta Quest (WebXR) 환경 — 직관적 단일 축 회전 및 데스크톱 1:1 매핑
| VR 컨트롤러 입력 | 물리 동작 및 피드백 | 상세 설명 |
| :--- | :--- | :--- |
| **왼쪽 Trigger 누르고 드래그** | **X축 회전 (Pitch / 상하 틸트)**<br>*(오렌지 레이저 & 햅틱 펄스)* | 왼손 검지 방아쇠를 쥔 채 컨트롤러를 상하로 움직이거나 손목을 위아래로 기울이면 오브젝트 시각적 중심을 기준으로 **X축 수직 틸트(Pitch)** 회전만 정밀하게 적용됩니다. |
| **오른쪽 Trigger 누르고 드래그** | **Y축 회전 (Yaw / 좌우 자전)**<br>*(보라색 레이저 & 햅틱 펄스)* | 오른손 검지 방아쇠를 쥔 채 컨트롤러를 좌우로 움직이거나 손목을 좌우로 비틀면 월드 수직축을 기준으로 **Y축 수평 자전(Yaw)** 회전만 정밀하게 적용됩니다. |
| **양손 Trigger 동시 사용** | **X축 & Y축 독립적 동시 회전** | 왼손으로 상하 각도(Pitch)를 잡고, 오른손으로 좌우 각도(Yaw)를 동시에 조작하여 롤(Roll) 뒤틀림이나 짐벌락 없이 직관적으로 원하는 뷰 각도를 완성할 수 있습니다. |
| **Grip(중지 손잡이) 쥐고 드래그** | **카메라 / 모델 위치 이동 (Pan)**<br>*(에메랄드 레이저 & 햅틱 펄스)* | 중지 그립 버튼을 쥔 채 컨트롤러를 움직이면 모델의 3차원 공간 위치를 자유롭게 이동시킵니다. (마우스 우클릭 드래그 대응) |
| **Grip 양손 동시 쥐기** | **양손 3D 핀치 줌 & 공간 이동**<br>*(양손 동시 햅틱 피드백)* | 양손 중지 그립을 동시에 쥐고 거리를 벌리거나 좁히면 오브젝트 로컬 중심 3D 스케일 확대/축소 및 양손 중심 이동을 수행합니다. |
| **Thumbstick(썸스틱) 앞/뒤 조작** | **카메라 확대 / 축소 (Zoom)** | 썸스틱을 앞으로 밀면 모델 확대 (Zoom In), 뒤로 당기면 축소 (Zoom Out)되어 마우스 휠 스크롤과 동일한 줌 조작감을 제공합니다. |
| **POI 핀 조준 후 Trigger 클릭** | **VR 3D 어노테이션 정보 카드 열람/닫기**<br>*(골드 레이저 & 클릭 햅틱)* | 레이저가 POI 핀을 가리키면 골드 색상으로 빛나며 손끝에 미세 햅틱이 전달되고, 방아쇠를 클릭하면 3D 정보 카드가 열리고 닫힙니다. |
| **B 버튼(우) 또는 Y 버튼(좌) 클릭** | **프리셋 모델 즉시 전환 (`Bonsai` ↔ `Dragon`)**<br>*(전환 햅틱 펄스)* | VR 세션을 종료하지 않고도 HMD 안에서 B/Y 버튼으로 프리셋 모델을 즉시 전환할 수 있습니다. |
...
> 1. **축 분리 회전 (Decoupled Single-Axis Rotation)**: 왼손은 X축(Pitch), 오른손은 Y축(Yaw)으로 회전축을 엄격히 분리하여 불필요한 롤(Roll) 회전 왜곡 및 짐벌락을 원천 차단했습니다.
> 2. **오브젝트 중심 고정 피벗**: 회전 및 확대/축소의 피벗 축이 항상 모델 자체의 시각적 중심(Visual Pivot)에 고정되어 있어 모델이 튀거나 시야 밖으로 날아가지 않습니다.
> 3. **물리 지수 감쇠 스무딩 (Exponential Damping)**: 인체의 미세 손떨림을 필터링하여 매끄럽고 안정적인 조작감을 제공합니다.
> 4. **손끝으로 전해지는 정밀 햅틱 (Haptic Affordance)**: 회전, 이동, 조준, 클릭, 모델 전환 등 모든 인터랙션 순간마다 고유한 진동 피드백을 전달하여 몰입감을 극대화합니다.
