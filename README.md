...
| 컨트롤러 입력 | 동작 |
| :--- | :--- |
| **한 손 트리거 (누른 채 이동/회전)** | 3DGS 모델을 잡고 이동 및 **오브젝트 중심 회전** |
| **양손 트리거 (동시 누른 채 간격 조절)** | 두 손 사이 거리로 **오브젝트 중심 줌인 / 줌아웃(확대/축소)** 및 회전 |
| **트리거 단발 클릭 (레이저로 POI 핀 조준)** | **VR 세션 내 3D 어노테이션 정보 카드 열람 / 닫기 토글** |

> **안내**: 썸스틱 및 그립 버튼의 복잡한 조작을 완전히 배제하고, 마우스처럼 직관적인 **트리거 단독 조작계**로 구성하여 조작 혼선을 방지했습니다. 또한 모든 회전 및 스케일 변환의 피벗 축은 월드 원점이 아닌 **오브젝트 자체의 중심**에 고정됩니다.
